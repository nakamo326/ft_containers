...
public:
  typedef AVLnode<K, V>                  node_type;
  typedef node_type*                     node_pointer;
  typedef std::allocator<AVLnode<K, V> > node_allocator;
...
public:
  AVLtree() : root_(NULL), size_(0), comp_(Comp()), alloc_(node_allocator()) {
    header_ = new node_type(K(), V());
  }
  ~AVLtree() { destroyTree(header_); }

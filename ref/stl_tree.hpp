...
namespace std _GLIBCXX_VISIBILITY(default) {
  ...
  struct _Rb_tree_iterator {
    ...
    friend bool operator==(const _Self& __x, const _Self& __y) { return __x._M_node == __y._M_node; }
  ...
  template <typename _Key,
            typename _Val,  // val == pair<const _key, _mapped_type>
  ...
  // clang-format on
  class _Rb_tree {
    typedef typename __gnu_cxx::__alloc_traits<_Alloc>::template rebind<_Rb_tree_node<_Val>>::other
                                                       _Node_allocator;
  ...
  protected:
    ...
    static const _Key&      _S_key(_Const_Link_type __x) { return _KeyOfValue()(*__x->_M_valptr()); }
  ...
  public:
    ...
    friend bool _GLIBCXX_DEPRECATED operator>(const _Rb_tree& __x, const _Rb_tree& __y) { return __y < __x; }
  ...
  pair<typename _Rb_tree<_Key, _Val, _KeyOfValue, _Compare, _Alloc>::_Base_ptr,
       typename _Rb_tree<_Key, _Val, _KeyOfValue, _Compare, _Alloc>::_Base_ptr>
  _Rb_tree<_Key, _Val, _KeyOfValue, _Compare, _Alloc>::_M_get_insert_hint_equal_pos(const_iterator __position,
                                                                                    const key_type& __k) {
    iterator                           __pos = __position._M_const_cast();
    typedef pair<_Base_ptr, _Base_ptr> _Res;
  ...
  void _Rb_tree<_Key, _Val, _KeyOfValue, _Compare, _Alloc>::_M_erase_aux(const_iterator __position) {
    _Link_type __y = static_cast<_Link_type>(
        _Rb_tree_rebalance_for_erase(const_cast<_Base_ptr>(__position._M_node), this->_M_impl._M_header));
    _M_drop_node(__y);
    --_M_impl._M_node_count;
  }
  ...
  bool _Rb_tree<_Key, _Val, _KeyOfValue, _Compare, _Alloc>::__rb_verify() const {
    if (_M_impl._M_node_count == 0 || begin() == end())
      return _M_impl._M_node_count == 0 && begin() == end() && this->_M_impl._M_header._M_left == _M_end() &&
